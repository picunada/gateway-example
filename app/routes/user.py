...
async def get_one(
        id: str,
        db: Optional[MongoDatabase] = Depends(get_database)) -> UserInDb:
    mongo = db.client
...
async def insert(
    ...
    user = UserInDb(**user.model_dump(), hashed_password=hashed_password, role=Roles.default)

    id = users.insert_one(user.model_dump()).inserted_id
...
async def put(
    ...
    mongo = db.client
    users = mongo.get_database("mt-services")["users"]
    user = users.update_one(ObjectId(id), user.model_dump())
    user = users.find_one(user.upserted_id)
    user = UserInDb(**user)
    return user
...
async def delete(
        id: str,
        db: Optional[MongoDatabase] = Depends(get_database)) -> JSONResponse:
    mongo = db.client
    users = mongo.get_database("mt-services")["users"]
